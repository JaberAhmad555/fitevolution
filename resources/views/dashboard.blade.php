<x-app-layout>
    ...
    {{-- The entire dashboard is one unified Alpine.js component --}}
    <div class="py-12" x-data="fitnessDashboard()">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-gray-800/90 backdrop-blur-sm shadow-2xl sm:rounded-2xl p-6 md:p-8">
                ...
                <form method="POST" action="{{ route('workouts.store') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
                    ...
                    <!-- Workout Type -->
                    <div>
                        <x-input-label for="type-select" :value="__('Workout Type')" class="text-gray-300"/>
                        <select id="type-select" name="type" x-model="workoutType" class="block mt-1 w-full border-gray-600 bg-gray-900 text-gray-300 rounded-md shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div><x-input-label for="workout_date" :value="__('Date of Workout')" class="text-gray-300"/><x-text-input id="workout_date" class="block mt-1 w-full border-gray-600 bg-gray-900 text-gray-300" type="date" name="workout_date" :value="old('workout_date')" required /></div>
                        <div><x-input-label for="duration_minutes" :value="__('Total Duration (in minutes)')" class="text-gray-300"/><x-text-input id="duration_minutes" class="block mt-1 w-full border-gray-600 bg-gray-900 text-gray-300" type="number" name="duration_minutes" :value="old('duration_minutes')" required /></div>
                    </div>

                    <!-- Dynamic Fields: Running/Cycling with Map -->
                    <div x-show="workoutType === 'running' || workoutType === 'cycling'" class="space-y-4">
                        <div x-data="{ isDistanceLocked: false }">
                            <label for="distance_km" class="block text-sm font-medium text-gray-300">Distance (in km)</label>
                            <div class="mt-1 flex rounded-md shadow-sm">
                                <input type="text" name="details[distance_km]" id="distance_km" x-bind:readonly="isDistanceLocked" class="block w-full border-gray-600 bg-gray-900 text-gray-300 rounded-l-md" placeholder="Type or plot on map">
                                <button type="button" @click="isDistanceLocked = !isDistanceLocked" class="relative -ml-px inline-flex items-center space-x-2 rounded-r-md border border-gray-600 px-4 py-2 text-sm font-medium text-gray-300 hover:bg-gray-700">
                        ...
                        <div>
                            ...
                            <div id="map-plotter" x-ref="mapContainer" class="mt-2 h-80 w-full rounded-lg z-0"></div>
                            <div class="mt-2"><button type="button" @click="$dispatch('reset-route')" class="text-sm text-indigo-400 hover:underline">Reset Route</button></div>
                            <input type="hidden" name="route" id="route-data-input">
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end">
                        <x-primary-button>{{ __('Save Workout') }}</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
    ...
    function fitnessDashboard() {
        return {
            ...
            workoutType: 'running',
            map: null, startPoint: null, endPoint: null, startMarker: null, endMarker: null, routeLine: null,

            // INIT method to set up all listeners
            init() {
                this.$watch('workoutType', () => this.handleMapVisibility());
                this.handleMapVisibility();
            },

            // Method for the Mood Suggester
            updateSuggestion() {
                this.suggestion = this.suggestions[this.selectedMood] || {};
                if (this.suggestion.type) {
                    this.workoutType = this.suggestion.type.toLowerCase();
                }
            },

            // Methods for the Map Plotter
            handleMapVisibility() {
                if (this.workoutType === 'running' || this.workoutType === 'cycling') {
            ...
            initializeMap() {
                ...
                navigator.geolocation.getCurrentPosition(pos => {
                    this.setupMap([pos.coords.latitude, pos.coords.longitude]);
                }, () => { this.setupMap([51.505, -0.09]); });
            },
            setupMap(center) {
                this.map = L.map(this.$refs.mapContainer).setView(center, 13);
                L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(this.map);
                this.map.on('click', (e) => this.onMapClick(e));
                this.$el.addEventListener('reset-route', () => this.resetRoute());
            },
            onMapClick(e) {
                if (!this.startPoint) {
                    this.startPoint = e.latlng;
                    if (this.startMarker) this.map.removeLayer(this.startMarker);
                    this.startMarker = L.marker(this.startPoint).addTo(this.map);
                } else if (!this.endPoint) {
                    this.endPoint = e.latlng;
                    if (this.endMarker) this.map.removeLayer(this.endMarker);
                    this.endMarker = L.marker(this.endPoint).addTo(this.map);
                    this.drawAndCalculate();
                }
            },
            drawAndCalculate() {
                if (!this.startPoint || !this.endPoint) return;
                if (this.routeLine) this.map.removeLayer(this.routeLine);
                this.routeLine = L.polyline([this.startPoint, this.endPoint], {color: 'blue'}).addTo(this.map);
                this.map.fitBounds(this.routeLine.getBounds().pad(0.1));
                const distanceKm = (this.startPoint.distanceTo(this.endPoint) / 1000).toFixed(2);
                const routeArray = [[this.startPoint.lat, this.startPoint.lng], [this.endPoint.lat, this.endPoint.lng]];
                const distanceInput = document.getElementById('distance_km');
                const routeInput = document.getElementById('route-data-input');
                const formAlpineData = distanceInput.closest('[x-data]').__x;
                if (!formAlpineData.isDistanceLocked) {
                    distanceInput.value = distanceKm;
                }
                routeInput.value = JSON.stringify(routeArray);
            },
            resetRoute() {
                if(this.startMarker) this.map.removeLayer(this.startMarker);
                if(this.endMarker) this.map.removeLayer(this.endMarker);
                if(this.routeLine) this.map.removeLayer(this.routeLine);
                this.startPoint = null; this.endPoint = null; this.startMarker = null; this.endMarker = null; this.routeLine = null;
                document.getElementById('distance_km').value = '';
                document.getElementById('route-data-input').value = '';
            }
        }
    }
    </script>
